...
if [ -n "$CERT_ARN" ];
then
  echo -e "\nRegistering certificate with ALB for: $CERTBOT_DOMAIN [$CERTBOT_ALB_NAME] [$CERTBOT_ALB_ARN]\n"

  aws elbv2 add-listener-certificates --listener-arn $CERTBOT_ALB_ARN --certificates CertificateArn=$CERT_ARN
else
